 ...
 * import { InMemoryBackendConfig, InMemoryBackendService, SEEDDATA } from
 *   "../in-memory-backend/in-memory-backend.service";
...
export class InMemoryBackendService {
  ...
  constructor(@Inject(SEED_DATA) private seedData:InMemoryDbService,
  ...
  public createConnection(req:Request):Connection {
   ...
   */
  protected commands(reqInfo:IReqInfo):ResponseOptions {
  ...
  protected delete({id, collection, collectionName, headers /*, req */}: IReqInfo):ResponseOptions {
    if (!id) {
  ...
  protected get({id, query, collection, collectionName, headers}: IReqInfo):ResponseOptions {
    let data = collection;
    if (id) {
    ...
    if (data instanceof Array) {
      let pdata = {
        ...
        nav: {size: 10, next: 2, prev: null, count: data.length, pages: Math.ceil(data.length / 10)},
  ...
  protected getLocation(href:string):any {
    let l = document.createElement("a");
    l.href = href;
    return l;
  };

  protected indexOf(collection:any[], id:number):any {
  ...
  protected parseUrl(url:string):any {
    try {
  ...
  protected post({collection, /* collectionName, */ headers, id, req, resourceUrl}: IReqInfo):ResponseOptions {
    ...
    if (existingIx > -1) {
      ...
      return new ResponseOptions({
        headers: headers,
        body: this.clone(item),
        status: STATUS.CREATED
      });
    }
  }

  protected put({id, collection, collectionName, headers, req}: IReqInfo):ResponseOptions {
    let item = JSON.parse(<string>req.text());
    if (!id) {
      return this.createErrorResponse(STATUS.NOT_FOUND, `Missing "${collectionName}" id`);
    }
